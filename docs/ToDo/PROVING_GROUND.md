...
- **The mesh faces its velocity, not the camera.** The first attempt at "running backwards"
  ...
  **That stopped being safe once the rifle was socketed** (A3). The gun points wherever the body
  ...
  not follow the mouse at all. Facing now has two modes in `Player:Animate`:

  - **Aiming** — cursor captured and standing, or within `AIM_HOLD` (0.8 s) of a shot: the body
    faces the camera's yaw, eased at 20/s.
  - **Otherwise** — the velocity rule above, unchanged.

  Aiming while moving has no right clip. A backwards move in aim mode plays the forward clip at
  negative `Speed` (`AnimationSystem` wraps past zero to the clip's end) — the moon-walk rejected
  above, now accepted as the lesser evil against a forward stride going backwards. A sideways move
  in aim mode slides. Both are content gaps: strafe and backpedal rifle clips.
