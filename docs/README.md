...
| [Assets](engine/assets.md)                      | AssetManager, handles, `.meta` sidecars and the asset scan, mesh import (cgltf), the binary mesh cache, and `.gmat` material assets                        |
...
- [`ASSET_PIPELINE_ROADMAP.md`](toDo&done/ASSET_PIPELINE_ROADMAP.md) — the asset pipeline + resource management milestone, drawn from BlankEngine's `AssetService`/`ResourceService` design (`.meta` sidecars, a typed manager registry with weak caches, `AssetRef<T>`, compiled outputs with epoch invalidation, async loading, hot reload; **Phase 1 done** — `.meta` sidecars and the scan-derived index are live in [assets.md](engine/assets.md); Phases 2–6 planned, and it deliberately excludes most of BlankEngine: read the scope assessment and "Not doing" first)
...
- [`REFLECTION_ROADMAP.md`](toDo&done/REFLECTION_ROADMAP.md) — member-level reflection on `entt::meta` for the generic inspector, generic serializer, and prefab per-property overrides (**R1 done** — all 23 components registered and self-validating, see [scene.md](engine/scene.md#member-reflection); R2–R4 are coarse sketches, sequenced after asset Phases 1–3)
