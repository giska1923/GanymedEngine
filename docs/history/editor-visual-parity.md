# Editor Visual Parity — the BlankEngine editor look

Goal: make GanymedEditor read as the same _class_ of tool as the BlankEngine (WoT HEAT 1.3 / Engine
...
Where a piece of BlankEngine furniture has no backing feature in Ganymed (a Profiler tab, a Validation
...
| Palette       | **BlankEngine's system, Ganymed's accent.** Same neutral ramp, same zero-rounding geometry, same semantic roles; `#F7A356` orange is replaced by a violet derived from the app icon.                          |
...
| Token           | Hex       | Where BlankEngine uses it                                                                                                                                          |
| --------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| `Border`        | `#1A1A1A` | Row separators, panel-to-panel gutters (1 px). Note: same value as `ChromeBg` — BlankEngine separates panels with _chrome-coloured gaps_, not with lighter borders |
...
| Metric                | BlankEngine                                                                                | Ganymed today                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
...
The app icon's violet is `#7B43C2` (hue 266°, relative luminance **0.121**). BlankEngine's orange sits
...
1. **Lighten the fill to a lilac and keep BlankEngine's dark glyph.** `#B182ED` fill with `#1A1A1A`
   text is 6.0:1; `#C39BFF` with `#101010` is 8.6:1, which is closest to the orange's feel.
   Preserves the _structure_ of the treatment (bright fill, dark glyph) — a loud, unmistakable
   selection.
2. Keep the fill dark (`#7B43C2`) and use light glyphs (6.1:1 against white). Readable, but it
   inverts the treatment: the selected row becomes _darker_ than the panel, which reads as
...
| `AccentText`   | `#B07BF4` | Accent-coloured _text_ on a panel background (4.3:1) — an active label with no fill |
...
- **Flat struct of `ImU32`, no inheritance, no registry, no runtime theme editor.** One `Apply()`
  with one call site. A theming _framework_ is the classic editor-tooling over-build; Unity and
...
- **Two presets ship**: `Ganymed` (the default — BlankEngine ramp + violet accent) and `Coldwar`
...
style.ScrollbarRounding = 0.0f;                  // BlankEngine is square everywhere
...
style.TabBarOverlineSize = 0.0f;                 // BlankEngine's active tab is a bg change only
...
| `Header`, `HeaderHovered`, `HeaderActive`                                   | `ChromeBg` / lighter / `Accent`                | **Also darker** than the window — BlankEngine's inspector component headers are `#1A1A1A` on `#313131`. ImGui's default is the opposite |
...
| `ScrollbarBg`                                                               | `SurfaceBg` (opaque)                           | BlankEngine's track is invisible; only the grab shows                                                                                   |
...
narrow left dock BlankEngine uses.

**Toolbar content**, matching BlankEngine's grouping: mode tools (select / translate / rotate / scale,
...
_fixes_ something: the gizmo mode is currently invisible unless you remember which key you pressed.
...
BlankEngine's outliner is the panel that most defines the look. Five changes, in order of impact:
...
2. **Column-header row**: `Name` | eye | lock | link. BlankEngine's is a `#272727` strip with three
   ...
   editor-side `std::unordered_set<UUID>` per flag, _not_ a component, for v1. Visibility that does
...
4. **Link-coloured labels.** BlankEngine draws entity names in `#589FFD`. Reserve `TextPrimary` for
...
override `*`, `OverflowMenuButton` → Remove). `GetActiveID` still runs _after_ the header.
...
BlankEngine's inspector is a stack of **full-width `#1A1A1A` rows** on a `#313131` panel: chevron, type
...
- `Add Component` becomes a full-width accent-outlined button at the bottom of the stack, as in
  BlankEngine, instead of the current inline popup trigger.
...
(`editor.md` → "The commit boundary"). A hand-drawn header adds _items_ to that section. The header's
...
_files_). Search filters that index recursively (lowercase `find` on the filename); empty query
...
The largest single panel job. BlankEngine's has four regions Ganymed lacks:
...
3. **Folder tree sidebar**, split from the grid. BlankEngine's is a fixed-width resizable pane. Use a
...
and cannot do the hover/selection cell fill BlankEngine's grid shows.
...
right: live counters. BlankEngine shows `WoT HEAT 1.3 · Engine 0.9.26.WIP · Release · Default ·
...
BlankEngine's dropdown set. Map to what exists; omit the rest:

| BlankEngine control | Ganymed                                                                                                                                                         |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
`GetCursorScreenPos()` _after_ the row would be drawn (`EditorLayer.cpp:447-453`) — so they stay
...
**The document-tab question.** BlankEngine's title bar tabs are _open documents_ —
...
BlankEngine's active tab. It looks right, it is honest, and it becomes a real tab bar the day
multi-document editing exists. Do not build fake tabs.

**Hit-test exclusion** is the fiddly part: the strip is draggable _except_ where an ImGui item is
...
|                                                                                 |                                                                                                                                                              |
| ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Log / Profiler / Statistics / Systems / Command History / Validation Log panels | Scope decision. Several need engine-side data sources that do not exist; the rest would be empty windows that make the editor look _less_ finished, not more |
...
- **Content density.** A good part of why the BlankEngine screenshot looks like a working tool is that
  it _is_ one: 3620 entities in the outliner, a real terrain in the viewport, 24 asset folders. An
...
3. **Accent** — **`#B182ED`** (lilac fill, dark glyph, keeps BlankEngine's treatment). `#7B43C2` with
