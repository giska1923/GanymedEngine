...
namespace GanymedE {
	EditorLayer::EditorLayer()
		: Layer("EditorLayer"), m_CameraController((float)DEFAULT_WINDOW_WIDTH / (float)DEFAULT_WINDOW_HEIGHT), m_SquareColor({ 0.2f, 0.3f, 0.8f, 1.0f })
	{
	}
	...
	void EditorLayer::OnUpdate(GanymedE::Timestep ts)
	{
		GE_PROFILE_FUNCTION();
		...
		if (dockingEnabled)
		{
			...
			if (ImGui::BeginMenuBar())
			{
				if (ImGui::BeginMenu("File"))
				{
					...
					if (ImGui::MenuItem("Exit")) GanymedE::Application::Get().Close();
					ImGui::EndMenu();
				}
				ImGui::EndMenuBar();
			}
			...
			ImGui::Image((void*)textureID, ImVec2{ DEFAULT_WINDOW_WIDTH , DEFAULT_WINDOW_HEIGHT }, ImVec2{ 0,1 }, ImVec2{ 1, 0 });
			ImGui::End();
	...
	void EditorLayer::OnEvent(GanymedE::Event& e)
	{
